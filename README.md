...
2. 没有状态保存，我写项目现在都是 MVVM 的，数据都放在 ViewModel 层，所以不用关心这个。Compose 项目直接给 Activity 加这个，Compose 自己本身就会自适应 UI 变化不用管, 传统项目老老实实用 XPopup 去
    ```text
    android:configChanges="orientation|screenSize|screenLayout|keyboardHidden|uiMode|locale|layoutDirection|smallestScreenSize"
